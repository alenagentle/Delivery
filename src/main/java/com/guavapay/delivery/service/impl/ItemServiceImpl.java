...
import com.guavapay.delivery.exception.NotEnoughProductException;
...
public class ItemServiceImpl implements ItemService {
    ...
    public ItemResponse createItem(ItemRequest itemRequest) {
        ...
        validateProductAmount(item);
        Item savedItem = itemRepository.save(item);
        log.info("New item saved. Id = {}", savedItem.getId());
        return itemMapper.mapToResponse(savedItem);
    }

    private void validateProductAmount(Item item) {
        if( item.getAmount() > item.getProduct().getAmount())
            throw new NotEnoughProductException("There is no such amount of the product in stock");
    }
